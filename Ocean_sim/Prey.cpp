...
void Prey::move_along_the_route(std::vector<std::vector<std::shared_ptr<EmptyEntity>>>& OceanGrid, PriorityEntityTuple& new_position)
{
    if (time_to_live)
    {
        --time_to_live;
...
void Prey::move_obj(std::vector<std::vector<std::shared_ptr<EmptyEntity>>>& OceanGrid)
{
    ...
    if (coord_y + 1 < OceanGrid[0].size()) {
        ...
        int priority = OceanGrid[x][y]->get_priority_prey();
